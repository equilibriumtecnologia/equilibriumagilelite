...
serve(async (req) => {
  ...
  try {
    ...
    // Validate it belongs to a product with plan_slug metadata
    const product = await stripe.products.retrieve(price.product as string);
    if (!product.metadata?.plan_slug) throw new Error("Invalid product");
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        user_id: user.id,
        plan_slug: product.metadata.plan_slug,
      },
      subscription_data: {
        metadata: {
          user_id: user.id,
